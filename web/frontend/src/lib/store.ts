...
// --- CONFIGURATION ---
// IMPORTANT: Pour la version WEB avec backend Node.js, nous utilisons les variables d'environnement
// ou des chemins relatifs si le backend est sur le même domaine.
const API_BASE = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000/api";
const MEDIA_BASE = process.env.NEXT_PUBLIC_MEDIA_URL || "http://localhost:5000";
...
class GSIStoreClass {
  ...
  private async apiCall(endpoint: string, method = 'GET', body?: any, redirectCount = 0): Promise<any> {
    ...
    try {
      ...
      // Add Admin Auth if present in session (for /admincreat or Admin Portal)
      if (typeof window !== 'undefined') {
  ...
  getStudentQrData(user: User): string {
    const info = {
      ...
      f: user.filiere,
      l: user.niveau,
      v: `${typeof window !== 'undefined' ? window.location.origin : 'https://groupegsi.mg'}/presence?id=${user.id}`
    };
